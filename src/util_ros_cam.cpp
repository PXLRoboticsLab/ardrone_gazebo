...
namespace gazebo
{
   ...
   void GazeboRosCameraUtils::LoadThread()
   {
	 ...
	 return;
      ...
      this->parentSensor_->SetActive(false);
      // ROS_INFO("-----------------------------------------------------------");
      // ROS_INFO_STREAM("robot_namespace_: " << this->robot_namespace_);
      // ROS_INFO_STREAM("camera_name_: " << this->camera_name_);
      // ROS_INFO("-----------------------------------------------------------");
      // The string parameter doesn't do a thing!
      // Solution: Add the robot_namespace_, etc. when advertising!
      // TOOD: Fix this!
      // TODO: When there isn't a namespace provided, set the namespace to /ardrone! 
...
//  }
      ...
      this->image_pub_ = this->itnode_->advertise(this->robot_namespace_ + "/" + this->image_topic_name_, 2,
      ...
      // of image_pipeline.
      ros::AdvertiseOptions cio = ros::AdvertiseOptions::create<sensor_msgs::CameraInfo>(this->robot_namespace_ + "/" + this->camera_info_topic_name_, 2,
											 boost::bind(&GazeboRosCameraUtils::ImageConnect, this),
											 boost::bind(&GazeboRosCameraUtils::ImageDisconnect, this),
											 ros::VoidPtr(), &this->camera_queue_);
...
// Initialize the controller
   void GazeboRosCameraUtils::Init()
   {
	 ...
	 this->cy_ = (static_cast<double>(this->height_) + 1.0) /2.0;
      ...
      double computed_focal_length = (static_cast<double>(this->width_))
